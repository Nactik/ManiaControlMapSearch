...
class MapList implements ManialinkPageAnswerListener, CallbackListener {
	...
	public function showMapList(Player $player, $mapList = null, $pageIndex = -1) {
		...
		$chunkIndex     = $this->getChunkIndexFromPageNumber($pageIndex); //s'incrémente de 1 toute les 2 pages
		$mapsBeginIndex = $this->getChunkMapsBeginIndex($chunkIndex); //s'incremente de 26 tout les 2 pages
		...
		if (!is_array($mapList)) {
			...
			//We store the mapList for paging
		...
		//OLD : $totalMapsCount = $this->maniaControl->getMapManager()->getMapsCount();
	...
	public function searchByAuthor(array $callback) {
		...
		// searchString is ok, the problem is not here
